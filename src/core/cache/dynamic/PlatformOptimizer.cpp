...
void PlatformOptimizer::adjustConfigForHardware(CacheConfig& config) const {
    try {
        ...
        if (config.maxSize == 0) {
            config.maxSize = 1024 * 1024 * 100; // 100 MB по умолчанию
        }
        if (config.maxEntries == 0) {
            config.maxEntries = 10000; // 10000 записей по умолчанию
        }
        if (config.entryLifetime.count() <= 0) {
            config.entryLifetime = std::chrono::seconds(3600); // 1 час по умолчанию
        }
