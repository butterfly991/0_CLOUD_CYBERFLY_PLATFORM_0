...
void ThreadPool::waitForCompletion() {
    try {
        while (true) {
            ...
            // Используем wait_for с таймаутом для предотвращения deadlock
            if (pImpl->condition.wait_for(lock, std::chrono::seconds(5)) == std::cv_status::timeout) {
                spdlog::get("threadpool")->warn("Таймаут ожидания завершения задач");
                break;
            }
        }
